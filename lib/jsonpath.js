   ...
   var P = {
      ...
      trace: function(expr, val, path) {
         ...
         if (val && val.hasOwnProperty(loc)) // simple case, directly follow property
            ...
            if (path[path.length-1] === 'extension') {
               var filteredExtensions = [];
               for (var j = 0; j < val.length; j += 1) {
                  if (loc.indexOf(val[j].url + "\"") > -1) {
                     filteredExtensions.push(val[j]);
                  }
               }
               P.walk("*", x, filteredExtensions, path, function(m,l,x,v,p) {
                  addRet(P.trace(unshift(m, x), v, p)); });
            } else {
               P.walk(loc, x, val, path, function(m,l,x,v,p) {
